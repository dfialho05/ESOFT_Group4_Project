...
public class ArquivoSessoesPag {
    ...
    public ArquivoSessoesPag(Cinema cinema, Runnable voltarCallback) {
        this.cinema = cinema;
    ...
    private JPanel createSessaoCard(Sessao sessao) {
        ...
        ativarButton.addActionListener(e -> {
            if (mainPanel != null) {
                int confirm = JOptionPane.showConfirmDialog(mainPanel, "Tem a certeza que deseja ativar esta sessão?", "Confirmar Ativação", JOptionPane.YES_NO_OPTION);
                if (confirm == JOptionPane.YES_OPTION) {
                    sessao.setAtivo(true);
                    JOptionPane.showMessageDialog(mainPanel, "Sessão ativada com sucesso.", "Sucesso", JOptionPane.INFORMATION_MESSAGE);
                    loadSessoes();
                }
            }
        });
